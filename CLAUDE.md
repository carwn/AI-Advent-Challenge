...
`RAGAgent` answers questions about SwiftUI API using a built-in knowledge base (~8400 chunks from Apple docs). Before each LLM call, `send(_:)` calls `RAGService.buildContext(for:topK:)` to retrieve the top-3 most relevant chunks via cosine similarity (threshold `minScore: 0.25`). Each question is handled **statelessly** via `sendMessage.execute(systemPrompt:userMessage:...)` — no conversation history is sent to the LLM, only the system prompt + current question (with optional RAG context). The original user message is appended to `conversation` directly; if RAG context was found, it is added as a `summaryUsage` message prefixed with "📚" (icon: `book.pages`). The assistant reply with token counts and response time is appended manually. If no RAG context is found, the question is still sent via the same stateless call without augmentation. Uses `CoreMLEmbeddingService` with BAAI/bge-small-en-v1.5 (offline, Neural Engine). Resources live in `Infrastructure/RAG/`: `chunks.json` and `vocab.txt` (auto-included by sync group), `BAAI_bge-small-en-v1.5.mlpackage` (explicit PBXFileReference, compiled to `.mlmodelc`). Stores `agentSystemPrompt` and `agentConversationId` as private properties for direct use in `send()`.

**RAG toggle**: `isRAGEnabled: Bool = true` flag controls whether the knowledge base is used. Can be toggled via toolbar button in `ChatView` (book icon: green=on, grey=off) or by text commands: `rag on` / `/rag on` / `rag вкл` / `/rag вкл` and `rag off` / `/rag off` / `rag выкл` / `/rag выкл`. When disabled, `send()` still uses the stateless `sendMessage.execute(systemPrompt:userMessage:...)` call but without RAG context. Command handling in `send()` appends user+assistant messages without an LLM call. `ChatViewModel` exposes `@Published var ragEnabled` and `isRAGAgent`, syncs the flag back from the agent after each `send()` (to reflect command-based changes).
