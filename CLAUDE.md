...
**RAG toggle**: `isRAGEnabled: Bool = true` flag controls whether the knowledge base is used. Can be toggled via toolbar button in `ChatView` (book icon: green=on, grey=off) or by text commands: `rag on` / `/rag on` / `rag вкл` / `/rag вкл` and `rag off` / `/rag off` / `rag выкл` / `/rag выкл`. When disabled, `send()` calls `super.send(text)` directly. Command handling in `send()` appends user+assistant messages without an LLM call. `ChatViewModel` exposes `@Published var ragEnabled` and `isRAGAgent`, syncs the flag back from the agent after each `send()` (to reflect command-based changes).
